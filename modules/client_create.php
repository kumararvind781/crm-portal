...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $newCompanyId = !empty($_POST['company_id'])
    ...
    if ($id) {
        ...
        // Get the OLD company before changing it
        $oldClient = fetch_one(
            "SELECT company_id FROM clients WHERE id = ?",
            [$id]
        );

        $oldCompanyId = !empty($oldClient['company_id'])
            ? (int) $oldClient['company_id']
            : null;

        // If company is being changed, save OLD company
        if (
            $oldCompanyId &&
            $oldCompanyId !== $newCompanyId
        ) {
            execute_query(
                "INSERT IGNORE INTO client_old_companies
                ...
                <form method="post" enctype="multipart/form-data" class="row g-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="first_name" class="form-control"
                            value="<?php echo esc($client['first_name'] ?? ''); ?>" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Last Name</label>
                        <input type="text" name="last_name" class="form-control"
                            value="<?php echo esc($client['last_name'] ?? ''); ?>" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Designation</label>
                        <input type="text" name="designation" class="form-control"
                            value="<?php echo esc($client['designation'] ?? ''); ?>">
                    </div>

                    <div class="col-md-8">
                        <label class="form-label">Person Address</label>
                        <input type="text" name="person_address" class="form-control"
                            value="<?php echo esc($client['person_address'] ?? ''); ?>">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Company Name</label>
                        <select name="company_id" class="form-select">
                            <option value="">No Company / Not Associated</option>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="person_city" class="form-control"
                            value="<?php echo esc($client['person_city'] ?? ''); ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Person Country</label>
                        <input type="text" name="person_country" class="form-control"
                            value="<?php echo esc($client['person_country'] ?? ''); ?>">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Email ID</label>
                        <input type="email" name="email_id" class="form-control"
                            value="<?php echo esc($client['email_id'] ?? ''); ?>">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Business Email ID</label>
                        <input type="email" name="business_email" class="form-control"
                            value="<?php echo esc($client['business_email'] ?? ''); ?>">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Business Phone Number</label>
                        <input type="text" name="business_phone" class="form-control"
                            value="<?php echo esc($client['business_phone'] ?? ''); ?>">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">Phone Number</label>
                        <input type="text" name="phone_number" class="form-control"
                            value="<?php echo esc($client['phone_number'] ?? ''); ?>">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label">WhatsApp / Alternate Number</label>
                        <input type="text" name="whatsapp_number" class="form-control"
                            value="<?php echo esc($client['whatsapp_number'] ?? ''); ?>">
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="file-box h-100">
                            ...
                            <?php if (!empty($client['photo'])): ?>
                                <div class="mt-3"><img class="preview-thumb"
                                        src="<?php echo BASE_URL . esc($client['photo']); ?>" alt="Photo"></div>
                            <?php endif; ?>
                        </div>
                    </div>
                        ...
                        <div class="file-box h-100">
                            ...
                            <?php if (!empty($client['visiting_card'])): ?>
                                <div class="mt-3 small text-muted">
                                    <a href="<?php echo BASE_URL . esc($client['visiting_card']); ?>" target="_blank">View
                                        uploaded card</a>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="url" name="linkedin_profile_url" class="form-control"
                            value="<?php echo esc($client['linkedin_profile_url'] ?? ''); ?>">
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Event(s) Met At</label>
                        <input type="text" name="events_met_at" class="form-control"
                            value="<?php echo esc($client['events_met_at'] ?? ''); ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Referred By - Name</label>
                        <input type="text" name="referred_by_name" class="form-control"
                            value="<?php echo esc($client['referred_by_name'] ?? ''); ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Referred By - Company</label>
                        <input type="text" name="referred_by_company" class="form-control"
                            value="<?php echo esc($client['referred_by_company'] ?? ''); ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Referred By - Number</label>
                        <input type="text" name="referred_by_number" class="form-control"
                            value="<?php echo esc($client['referred_by_number'] ?? ''); ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Referred By - Email</label>
                        <input type="email" name="referred_by_email" class="form-control"
                            value="<?php echo esc($client['referred_by_email'] ?? ''); ?>">
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="notes" class="form-control"
                            value="<?php echo esc($client['notes'] ?? ''); ?>">
                    </div>

                    <div class="col-12">
                        <div class="actions pt-2">
                            <button class="btn btn-primary px-4"
                                type="submit"><?php echo $client ? 'Update Client' : 'Save Client'; ?></button>
                            <a class="btn btn-outline-secondary"
                                href="<?php echo BASE_URL; ?>modules/clients.php">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
